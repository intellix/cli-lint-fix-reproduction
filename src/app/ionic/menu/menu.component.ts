...
export class MenuComponent {
   ...
   */
  @Output() ionOpened: EventEmitter<boolean> = new EventEmitter<boolean>();

  /**
   * @output {event} Emitted when the menu has been closed.
   */
  @Output() ionClosed: EventEmitter<boolean> = new EventEmitter<boolean>();
  ...
  _swipeProgress(stepValue: number) {
    ...
    const drag = this.ionDrag;
    if (drag.observers.length > 0) {
      drag.emit(stepValue);
    }
  }
  ...
  setIsPanel(isPanel: boolean): MenuComponent {
