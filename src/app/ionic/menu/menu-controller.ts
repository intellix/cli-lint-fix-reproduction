...
export class MenuController {
  private menus: Array<MenuComponent> = [];
   ...
   */
  enable(shouldEnable: boolean, menuId?: string): MenuComponent {
    const menu = this.get(menuId);
    if (menu) {
   ...
   * @return {Menu}  Returns the instance of the menu, which is useful for chaining.
   */
  swipeEnable(shouldEnable: boolean, menuId?: string): MenuComponent {
    const menu = this.get(menuId);
    if (menu) {
   ...
   */
  get(menuId?: string): MenuComponent {
   ...
   * @private
   */
  _setActiveMenu(menu: MenuComponent) {
   ...
   * @private
   */
  static create(type: string, menuCmp: MenuComponent, plt: Platform) {
